...
function dsc_render_desktop_nav( $location = 'primary' ) {
	...
	if ( empty( $tree ) ) {
		$default = array(
			array( 'label' => __( 'Home', 'domesca-homes' ), 'url' => home_url( '/' ) ),
			array( 'label' => __( 'About Us', 'domesca-homes' ), 'url' => home_url( '/about/' ) ),
			array(
				'label'    => __( 'Services', 'domesca-homes' ),
				'url'      => home_url( '/services/' ),
				'children' => array(
					array( 'label' => __( 'Our Plans', 'domesca-homes' ), 'url' => home_url( '/our-plans/' ) ),
					array( 'label' => __( 'New Builds', 'domesca-homes' ), 'url' => home_url( '/new-builds/' ) ),
					array( 'label' => __( 'Townhouse Developments', 'domesca-homes' ), 'url' => home_url( '/townhouse-developments/' ) ),
					array( 'label' => __( 'Multi-Unit Projects', 'domesca-homes' ), 'url' => home_url( '/multi-unit-projects/' ) ),
					array( 'label' => __( 'Extensions', 'domesca-homes' ), 'url' => home_url( '/extensions/' ) ),
					array( 'label' => __( 'Renovations', 'domesca-homes' ), 'url' => home_url( '/renovations/' ) ),
				),
			),
			array( 'label' => __( 'Our Plans', 'domesca-homes' ), 'url' => home_url( '/our-plans/' ) ),
			array( 'label' => __( 'Portfolio', 'domesca-homes' ), 'url' => home_url( '/portfolio/' ) ),
			array( 'label' => __( 'Contact Us', 'domesca-homes' ), 'url' => home_url( '/contact/' ) ),
		);
		echo '<nav class="nav" aria-label="Primary">';
		foreach ( $default as $item ) {
			echo '<div class="nav__item">';
			echo '<a class="nav__link" href="' . esc_url( $item['url'] ) . '">' . esc_html( $item['label'] );
...
function dsc_render_mobile_nav( $location = 'primary' ) {
	...
	if ( empty( $tree ) ) {
		...
		foreach ( $default as $i => $item ) {
			echo '<li>';
			if ( ! empty( $item['children'] ) ) {
				$panel = 'mnav-sub-' . absint( $i + 1 );
				echo '<div class="mnav__row">';
				echo '<a class="mnav__a mnav__link" href="' . esc_url( $item['url'] ) . '">' . esc_html( $item['label'] ) . '</a>';
				echo '<button class="mnav__pm" type="button" aria-expanded="false" aria-controls="' . esc_attr( $panel ) . '" aria-label="' . esc_attr( sprintf( __( 'Toggle %s submenu', 'domesca-homes' ), $item['label'] ) ) . '"><span class="mnav__pm-i" aria-hidden="true"></span></button>';
				echo '</div>';
				...
				echo '<a class="mnav__a" href="' . esc_url( $item['url'] ) . '">' . esc_html( $item['label'] ) . '</a>';
			}
			echo '</li>';
		}
		echo '</ul>';
...
function dsc_render_footer_columns( $location = 'footer' ) {
	...
	foreach ( $tree as $i => $column ) {
		...
		echo '<h3 class="ft__h"><button class="ft__toggle" type="button" data-ft-toggle aria-expanded="true" aria-controls="' . esc_attr( $panel ) . '">' . esc_html( $column->title ) . '<span class="ft__ic" aria-hidden="true"></span></button></h3>';
...
/**
 * Small icon helper for the "Your plans, or ours" route cards.
 */
function dsc_plan_icon( $icon = 'plans' ) {
	switch ( $icon ) {
		case 'pencil':
			return '<path d="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/>';
		case 'plans':
		default:
